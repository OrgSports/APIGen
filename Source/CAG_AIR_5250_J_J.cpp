...
CAG_AIR_5250_J_J::CAG_AIR_5250_J_J()
{
	resetVariableFlags();
}
...
void CAG_AIR_5250_J_J::generateHeader(CStringArray* pcsaHeader,
									  CStringArray* pcsaBody)
{
	...
	if (IDOK == XML5250Dlg.DoModal())
	{
		CString csLocalString;
		pcsaHeader->Add("/* 5250.XML Airlines SDK Configuration File");

		CString csCharCase, csAttribute, csLightPen;

		// if the character case option is specified as something other than mixed
		if ( -1 == XML5250Dlg.csList5250CharCase.Find( "Mixed" ) )	// default
		{
			if ( -1 != XML5250Dlg.csList5250CharCase.Find( "Lower" ) )
			{
				csCharCase="Lower";
			}
			else
			...
			// if the backspace option calls for deletion on backspace
			if ( -1 == XML5250Dlg.csList5250AttributeChoice.Find( "processed" ) ) // default
			{
				if ( -1 != XML5250Dlg.csList5250AttributeChoice.Find( "asterisks" ) )
				{
					csAttribute="Asterisk";
				}
				else
				{
					csAttribute="Dot";
				...
				// if the lightpen is to be enabled
				if ( XML5250Dlg.bCheck5250NumericFieldOverride )
				{
					if ( XML5250Dlg.bCheck5250DestructiveBackspace )
					{
						csLocalString.Format( "<ScreenTn5250 TextCase=\"%s\" AttributeStyle=\"%s\" BackSpaceKey=\"%s\" NumFldOverride=\"True\">", csCharCase, csAttribute );
					}
					else
					{
						csLocalString.Format( "<ScreenTn5250 TextCase=\"%s\" AttributeStyle=\"%s\" BackSpaceKey=\"%s\" NumFldOverride=\"True\" DestructiveBackspace=\"False\">", csCharCase, csAttribute );
					}
				}
				else
				{
					if ( XML5250Dlg.bCheck5250DestructiveBackspace )
					{
						csLocalString.Format( "<ScreenTn5250 TextCase=\"%s\" AttributeStyle=\"%s\" BackSpaceKey=\"%s\">", csCharCase, csAttribute );
					}
					else
					{
						csLocalString.Format( "<ScreenTn5250 TextCase=\"%s\" AttributeStyle=\"%s\" BackSpaceKey=\"%s\" DestructiveBackspace=\"False\">", csCharCase, csAttribute );
					}
				}
			}
			else
			{
				// character case option, but no backspace option specified
				// if the lightpen is to be enabled
				if ( XML5250Dlg.bCheck5250NumericFieldOverride )
				{
					if ( XML5250Dlg.bCheck5250DestructiveBackspace )
					{
						csLocalString.Format( "<ScreenTn5250 TextCase=\"%s\" NumFldOverride=\"True\">", csCharCase );
					}
					else
					{
						csLocalString.Format( "<ScreenTn5250 TextCase=\"%s\" NumFldOverride=\"True\" DestructiveBackspace=\"False\">", csCharCase );
					}
				}
				else
				{
					if ( XML5250Dlg.bCheck5250DestructiveBackspace )
					{
						csLocalString.Format( "<ScreenTn5250 TextCase=\"%s\">", csCharCase );
					}
					else
					{
						csLocalString.Format( "<ScreenTn5250 TextCase=\"%s\" DestructiveBackspace=\"False\">", csCharCase );
					}
				}
		...
		else
		{
			...
			if ( -1 == XML5250Dlg.csList5250AttributeChoice.Find( "processed" ) ) // default
			{
				...
				if ( XML5250Dlg.bCheck5250NumericFieldOverride )
				{
					if ( XML5250Dlg.bCheck5250DestructiveBackspace )
					{
						csLocalString.Format( "<ScreenTn5250 AttributeStyle=\"%s\" NumFldOverride=\"True\">", csAttribute );
					}
					else
					{
						csLocalString.Format( "<ScreenTn5250 AttributeStyle=\"%s\" NumFldOverride=\"True\" DestructiveBackspace=\"False\">", csAttribute );
					}
				}
				else
				{
					if ( XML5250Dlg.bCheck5250DestructiveBackspace )
					{
						csLocalString.Format( "<ScreenTn5250 AttributeStyle=\"%s\">", csAttribute );
					}
					else
					{
						csLocalString.Format( "<ScreenTn5250 AttributeStyle=\"%s\" DestructiveBackspace=\"False\">", csAttribute );
					}
				}
			}
			else
			{
				...
				if ( XML5250Dlg.bCheck5250NumericFieldOverride )
				{
					if ( XML5250Dlg.bCheck5250DestructiveBackspace )
					{
						csLocalString.Format( "<ScreenTn5250 NumFldOverride=\"True\">" );
					}
					else
					{
						csLocalString.Format( "<ScreenTn5250 NumFldOverride=\"True\" DestructiveBackspace=\"False\">" );
					}
				}
				else
				{
					if ( XML5250Dlg.bCheck5250DestructiveBackspace )
					{
						csLocalString.Format( "<ScreenTn5250>" );
					}
					else
					{
						csLocalString.Format( "<ScreenTn5250 DestructiveBackspace=\"False\">" );
					}
				}
		...
		pcsaHeader->Add("        <Tn5250con>");
		csLocalString.Format( "               <Tcp5250con HostName=\"%s\" PortNumber=\"%s\"\\>", XML5250Dlg.csEdit5250HostName, XML5250Dlg.csEdit5250PortNumber );
		pcsaHeader->Add(csLocalString);
		pcsaHeader->Add("        <\\Tn5250con>");
		csLocalString.Format( "        <ReadXlat XlateFileName=\"%s\"\\>", XML5250Dlg.csEdit5250XLateFileName );
		pcsaHeader->Add(csLocalString);
		pcsaHeader->Add("<\\ScreenTn5250>");
		pcsaHeader->Add(" *** END 5250.XML Airlines SDK Configuration File ***/ ");
		pcsaHeader->Add("");
		pcsaHeader->Add("");
	}   
	...
	pcsaHeader->Add("// ****************************************************************************");
	pcsaHeader->Add("//                Attachmate Airlines 5250 SDK");
	...
	pcsaHeader->Add("//    of Airlines SDK 5250 Applet Java method calls specified to the APIGen code");
	pcsaHeader->Add("//    generator.");
	pcsaHeader->Add("//");
	pcsaHeader->Add("//    These conditions must be met before this source file can be used.");
	pcsaHeader->Add("//       1. Either the class name (Air5250) or the file name must be changed as");
	pcsaHeader->Add("//          they must be the same.");
	pcsaHeader->Add("//       2. The Attachmate Airlines SDK must be installed using the keycode for 5250. Its");
	pcsaHeader->Add("//           class or jar files must be made accessible via the class path.");
	pcsaHeader->Add("//       3. If you specified the parameters for a 5250 session XML configuration file");
	...
	pcsaHeader->Add("//          to its own file and either place the new file named, \"5250.xml\" to the c:\\temp");
	...
	pcsaHeader->Add("         fis = new FileInputStream(\"C:\\temp\\5250.xml\");" );
	...
	pcsaHeader->Add("      ScreenTn5250 screen;" );
	pcsaHeader->Add("      String strConfig = getXmlConfig();" );
	pcsaHeader->Add("      System.out.println(\"DEBUG:    strConfig = \" + strConfig);" );
	pcsaHeader->Add("" );
	pcsaHeader->Add("      try" );
	pcsaHeader->Add("      {" );
	pcsaHeader->Add("         screen = (ScreenTn5250) sessLoader.requestScreenEx(\"\",\"\",strConfig,0);" );
...
void CAG_AIR_5250_J_J::appendBody(int nFunctionNum,
								  ...
								  CStringArray* pcsaBody)
{
	...
	switch ( nFunctionNum )
	{
		...
		case 5:		// IScreenUPG::addIScreenListener
		{
			...
			pcsaBody->Add("              ScreenTn5250 = (ScreenType) sessLoader.requestScreen(User,Password,WebView);");
			pcsaBody->Add("              if (ScreenTn5250 != null)");
			pcsaBody->Add("              {");
			pcsaBody->Add("                   ScreenTn5250.addIScreenListener(this);");
			pcsaBody->Add("                   if (ScreenTn5250.Open())");
			pcsaBody->Add("                   {");
			pcsaBody->Add("                   }");
			pcsaBody->Add("              }");
			pcsaBody->Add(" " );
		...
		{
			pcsaBody->Add("            ScreenTn5250 = (ScreenType) sessLoader.requestScreen(User,Password,WebView); //XXX declare vars");
			pcsaBody->Add("            if (ScreenTn5250 != null)");
			pcsaBody->Add("            {");
			pcsaBody->Add("                 ScreenTn5250.clearCapturedData();");
			pcsaBody->Add("            }");
			pcsaBody->Add(" " );
		}
		break;
		...
		case 45:		// IScreenUPG::setgetText_Display
{
		...
		case 58:	// SessionLoader::setTerminalType
		{
			...
			else if ( -1 != pcsaArguments->GetAt(1).Find( "5250" ) )
			{
				csViewOutStuff.Format( "            nType = 110; //5250" );
			}
			else if ( -1 != pcsaArguments->GetAt(1).Find( "5250" ) )
			{
				csViewOutStuff.Format( "            nType = 115; //5250" );
			}
...
void CAG_AIR_5250_J_J::finalize(CStringArray* pcsaBody)
{
	CString csViewOutStuff;
...
/****************************************************************************DC
	Function	:	CAG_AIR_5250_J_J::declareVariable()

	Purpose		:	Append variable declarations to the end of the header as 
					needed.

*****************************************************************************/
void CAG_AIR_5250_J_J::declareVariable(AG_VARIABLE agVariable,
									   CStringArray* pcsaHeader,
									   bool* bValue)
{
	CString csLocal;
...
void CAG_AIR_5250_J_J::checkDebug(bool b_Debug,
								  CString csFunctionName,
								  CStringArray* pcsaBody)
{
...
void CAG_AIR_5250_J_J::resetVariableFlags()
{
