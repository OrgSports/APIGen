...
/****************************************************************************DC
	Function	:	CAPIGenDlg::LoadCICSCOMArgumentArray()

	Purpose		:	Load the function arguments for CICS Bridge QACom Automation.

*****************************************************************************/
void CAPIGenDlg::LoadCICSCOMArgumentArray( int* nFunctionPosition, BOOL bPromptForValues )
{
	...
	else
	{
		switch ( *nFunctionPosition )
		{
			...
			case 1:		// ConnectBridge
			{
				...
				// terminal pool
				csaArguments.Add( "pool2" );

				// network name
				csaArguments.Add( "FOX" );

				// terminal facility like
			...
			case 3:		// FindString
			{
				...
				// in flags
				csaArguments.Add( "Ignore char case" );
			}
			break;
...
/****************************************************************************DC
	Function	:	CAPIGenDlg::ShowCICSCOMControls()

	Purpose		:	Show the controls for the CICS Bridge QACom Automation 
					function parameters.

*****************************************************************************/
void CAPIGenDlg::ShowCICSCOMControls( int nFunction )
{
	...
	switch ( nFunction )
	{
		...
		case 29:	// SetTraceName
		{
			csaLabels.Add("CICS Temp Storage Queue");
		}
		break;
	...
	{
		ShowMixedControls( CICS_COM_API, nFunction, &csaLabels );	
	}
