...
void CAG_VHI_VBN::generateHeader(CStringArray* pcsaHeader,
									 CStringArray* pcsaBody)
{
	...
	pcsaHeader->Add("'                  APIGen VB.Net Application");
	pcsaHeader->Add("'" );
	pcsaHeader->Add("'   This VB.Net code offers a freestanding module containing the set of Verastream");
	...
	pcsaHeader->Add("'   1) Ensure that Microsoft Visual Studio .Net is installed and available on your PC.");
	...
	pcsaHeader->Add("'   3) Create a new Visual Studio .Net project. The Standard \"Windows Application\" option");
	pcsaHeader->Add("'      is fine."); 
	pcsaHeader->Add("'   4) Click on the Project\\Add existing item menu item.  Browse to, and select this .vb file.");
	pcsaHeader->Add("'      After you click OK the module text should consist of a module declaration at the top, the");
	pcsaHeader->Add("'      contents of this file, and end with an \"End Module\" line.");
	pcsaHeader->Add("'   5) Insert the string \"Imports AppConnLib\" at the top of the project file, just above the");
	pcsaHeader->Add("'      Module line.");
	pcsaHeader->Add("'   6) The VHI Com object, AppConn.dll, should be registered on your system after");
	pcsaHeader->Add("'      you installed the VHI toolkit, but you will need to add it as a reference in your ");
	pcsaHeader->Add("'      Visual Studio .Net IDE.  To do so, choose the Project\\Add Reference menu item,");
	pcsaHeader->Add("'      choose the COM tab, and ensure that the checkbox next to the reference \"WRQ");
	pcsaHeader->Add("'      Verastream Host Integrator ?.?\" is checked. Version numbers will show in place");
	pcsaHeader->Add("'      of the ? wildcard characters. Currently the version is 5.5.");
	pcsaHeader->Add("'   7) Right click on the name of the module in the Solution Navigator window and choose the");
	pcsaHeader->Add("'      Properties option. Select \"Sub Main\" from the Startup object list control.");
	pcsaHeader->Add("'   8) You can now run this within the Visual Studio .Net IDE or you can compile it to run as");
...
void CAG_VHI_VBN::appendBody(int nFunctionNum,
...
void CAG_VHI_VBN::finalize(CStringArray* pcsaBody)
{
	pcsaBody->Add( "" );
	pcsaBody->Add( "" );
...
void CAG_VHI_VBN::declareVariable(AG_VARIABLE agVariable,
									  CStringArray* pcsaHeader,
									  bool* bValue)
{
	CString csLocal;
...
void CAG_VHI_VBN::checkDebug(bool b_Debug,
...
void CAG_VHI_VBN::resetVariableFlags()
{
