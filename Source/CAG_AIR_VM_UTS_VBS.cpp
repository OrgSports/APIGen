...
CAG_AIR_VM_UTS_VBS::CAG_AIR_VM_UTS_VBS()
{
	resetVariableFlags();
}
...
void CAG_AIR_VM_UTS_VBS::generateHeader(CStringArray* pcsaHeader,
									  CStringArray* pcsaBody)
{
	CString csHostNameString, csEnvironmentName, csTransport;
	if (IDOK == AirUTSHostnameDlg.DoModal())
	{
		csHostNameString = AirUTSHostnameDlg.csEditUTSHostAddress;
		if ( 0 == csHostNameString.GetLength() )
		{
			csHostNameString = "T319";
		}

		csEnvironmentName = AirUTSHostnameDlg.csEditUTSEnvironmentName;
		if ( 0 == csEnvironmentName.GetLength() )
		{
			csEnvironmentName = "UDSSRC";
		}

		csTransport = AirUTSHostnameDlg.csEditUTSTransport;
		if ( 0 == csTransport.GetLength() )
		{
			csTransport = "INT1";
		}
	}
	...
	pcsaHeader->Add("//");
	pcsaHeader->Add("//    To run the Express Viewer session configured and automated in this file you ");
	...
	pcsaHeader->Add("   codebase=\"UTS\"");
	pcsaHeader->Add("   code=\"com.attachmate.cin.frame.AtmApplet\"");
	pcsaHeader->Add("   id=\"UTSApplet\"");
	pcsaHeader->Add("   name=\"UTSApplet\"");
	pcsaHeader->Add("   width=\"0\"");
	pcsaHeader->Add("   height=\"0\"");
	pcsaHeader->Add("   archive=\"uts.jar\" VIEWASTEXT>");
	pcsaHeader->Add("   <PARAM name=\"Rows\" value=\"24\">");
	pcsaHeader->Add("   <PARAM name=\"Columns\"  value=\"80\">");
	pcsaHeader->Add("   <PARAM name=\"HtmlHelp\" value=\"hudhelp.htm\">");
	pcsaHeader->Add("   <PARAM name=\"Cabbase\" value=\"uts.cab\">");
	pcsaHeader->Add("   <PARAM name=\"HostType\" value=\"UTS\">");
	...
	csViewOutStuff.Format( "   <PARAM name=\"Transport\" value=\"%s\">", csTransport );
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "   <PARAM name=\"EnvironmentName\" value=\"%s\">", csEnvironmentName );
	pcsaHeader->Add(csViewOutStuff);

	pcsaHeader->Add("   <PARAM name=\"AutoConnect\" value=\"TRUE\">");

	pcsaHeader->Add("   <PARAM name=\"INT1.HREF\" value=\"UTSEnvs/\">");

	pcsaHeader->Add("   <PARAM name=\"SendReady\" value=\"FALSE\">");
	pcsaHeader->Add("   <PARAM name=\"ShortTextHICs\" value=\"FALSE\">");
	pcsaHeader->Add("   <PARAM name=\"ConnectString\" value=\"$$OPEN\">");
	pcsaHeader->Add("   <PARAM name=\"DisconnectString\" value=\"$$CLOSE\">");

	pcsaHeader->Add("   <PARAM name=\"INT1DeviceType\" value=\"UTS60\">");
	pcsaHeader->Add("   <PARAM name=\"TerminalID\" value=\"T319\">");
	...
	pcsaHeader->Add("   <PARAM name=\"BlinkText\" value=\"FALSE\">");
...
void CAG_AIR_VM_UTS_VBS::appendBody(int nFunctionNum,
								  ...
								  CStringArray* pcsaBody)
{
	...
	switch ( nFunctionNum )
	{
		...
		{
			pcsaBody->Add("        if (ScreenUTS != null)");
			pcsaBody->Add("        {");
			pcsaBody->Add("         ScreenUTS.clearCapturedData()");
			pcsaBody->Add("        }");
			pcsaBody->Add("" );
		}
		break;
		...
		case 1:		// IScreenUPG::fieldCount
		{
			...
			{
				nLocalField=1;
				csViewOutStuff.Format( "        nCount = ScreenUTS.fieldCount( %i )  'FIELDTYPE_PROTECTED", nLocalField );
			}
			else if ( -1 != pcsaArguments->GetAt(1).Find( "FIELDTYPE_UNPROTECTED" ) )
			{
				nLocalField=2;
				csViewOutStuff.Format( "        nCount = ScreenUTS.fieldCount( %i )  'FIELDTYPE_UNPROTECTED", nLocalField );
			}
			else
			{
				nLocalField=3;
				csViewOutStuff.Format( "        nCount = ScreenUTS.fieldCount( %i )  'FIELDTYPE_ANY", nLocalField );
			}
			pcsaBody->Add(csViewOutStuff);
			pcsaBody->Add(" " );
		}
		...
		case 2:		// IScreenUPG::findField
		{
			...
			pcsaBody->Add("        bRet = ScreenUTS.findField( nRow, nColumn, nMode, nType, bWrap, nOutRow, nOutColumn, nOutLength, nOutAttribute  )" );
			pcsaBody->Add(" " );
		}
		break;
		...
		case 3:	// IScreenUPG::findFieldEx
		{
			...
			pcsaBody->Add("" );
			pcsaBody->Add("        bRet = ScreenUTS.findFieldEx( nIndex, nMode, nType, bWrap, nOutRow, nOutColumn, nOutLength, nOutAttribute  )" );
			pcsaBody->Add("" );
		}
		break;
		...
		case 4:	// IScreenUPG::findString
		{
			...
			pcsaBody->Add("" );
			pcsaBody->Add("        bRet = ScreenUTS.findString( cText, nRow, nColumn )" );
			pcsaBody->Add(" " );
		...
		case 5:	// IScreenUPG::findStringEx
		{
			...
			pcsaBody->Add("" );
			pcsaBody->Add("        bRet = ScreenUTS.findStringEx( cText, nRow, nColumn, nEndRow, nEndColumn, 0, nWrap, nDir )" );
			pcsaBody->Add(" " );
		...
		{
			pcsaBody->Add("        ScreenUTS.focusTerminal()" );
			pcsaBody->Add(" " );
		}
		break;
		...
		case 7:	// IScreenUPG::getCapturedData
		{
			...
			pcsaBody->Add("        Buffer = ScreenUTS.getCapturedData()" );
			pcsaBody->Add(" " );
		...
		case 8:	// IScreenUPG::getColumns
		{
			...
			pcsaBody->Add("        nCount = ScreenUTS.getColumns()" );
		}
		break;
		...
		case 9:		// IScreenUPG::getConnectStatus
		{
			...
			pcsaBody->Add("        bRet = ScreenUTS.getConnectStatus()" );
		...
		case 10:		// IScreenUPG::getCursor
		{
			...
			pcsaBody->Add("        cursorObj = ScreenUTS.getCursor()   'todo" );	
			pcsaBody->Add(" " );
		}
		break;
		...
		case 11:		// IScreenUPG::getCursorColumn
		{
			...
			pcsaBody->Add("        nColumn = ScreenUTS.getCursorColumn()" );
			pcsaBody->Add("" );
		}
		break;
		...
		case 12:		// Screen::getCursorRow
		{
			...
			pcsaBody->Add("        nRow = ScreenUTS.getCursorRow()" );
			pcsaBody->Add("" );
		}
		break;
		...
		case 13:		// IScreenUPG::getDataCapture
		{
			...
			pcsaBody->Add("        bRet = ScreenUTS.getDataCapture()" );
			pcsaBody->Add("" );
		}
		break;
		...
		case 14:	// IScreenUPG::getExtPS
		{
			...
			pcsaBody->Add("        Buffer = ScreenUTS.getExtPS( nRow, nColumn, nLength )" );
			pcsaBody->Add("" );
		...
		case 15:	// IScreenUPG::getgetText_Display
		{
			...
			pcsaBody->Add("        bRet = ScreenUTS.getgetText_Display()" );
			pcsaBody->Add("" );
		...
		case 16:	// IScreenUPG::getKeyboardLocked
		{
			...
			pcsaBody->Add("        bKeyboardLocked = ScreenUTS.getKeyboardLocked()" );
			pcsaBody->Add("");
		...
		case 17:	// IScreenUPG::getMaxDataCaptureLength
		{
			...
			pcsaBody->Add("        nLength = ScreenUTS.getMaxDataCaptureLength()");
			pcsaBody->Add("");
		...
		case 18:	// IScreenUPG::getObjectVersion
		{
			...
			pcsaBody->Add("        nValue = ScreenUTS.getObjectVersion()" );
			pcsaBody->Add("");
		...
		case 19:	// IScreenUPG::getOIAContents
		{
			...
			pcsaBody->Add("        Buffer = ScreenUTS.getOIAContents()");
			pcsaBody->Add("");
		...
		case 20:	// IScreenUPG::getOIAText
		{
			...
			pcsaBody->Add("        Buffer = ScreenUTS.getOIAText()");
			pcsaBody->Add("");
		...
		case 21:	// IScreenUPG::getRows
		{
			...
			pcsaBody->Add("        nCount = ScreenUTS.getRows()");
			pcsaBody->Add("");
		...
		case 22:	// IScreenUPG::getScreenType
		{
			...
			csViewOutStuff.Format( "        nType = ScreenUTS.getScreenType()" );
			pcsaBody->Add(csViewOutStuff);
		}
		break;
		...
		case 23:	// Screen::getString
		{
			...
			csViewOutStuff.Format( "        cString = ScreenUTS.getString( nRow, nColumn, nLength )");
			pcsaBody->Add(csViewOutStuff);
		...
		case 24:	// IScreenUPG::getStringEx
		{
			...
			pcsaBody->Add("" );
			pcsaBody->Add("        cText = ScreenUTS.getStringEx( nRow, nColumn, nEndRow, nEndColumn, 0, nWrap, nAttribute, 0 )" );
			pcsaBody->Add(" " );
		...
		case 25:	// Screen::getText
		{
			...
			csViewOutStuff.Format( "        cText = ScreenUTS.getText()");
			pcsaBody->Add(csViewOutStuff);
		...
		case 26:	// IScreenUPG::hideTerminal
		{
			...
			csViewOutStuff.Format( "        ScreenUTS.hideTerminal()");
			pcsaBody->Add(csViewOutStuff);
		...
		case 27:	// IScreenUPG::isKeyboardLocked
		{
			...
			csViewOutStuff.Format( "        bKeyboardLocked = ScreenUTS.isKeyboardLocked()");
			pcsaBody->Add(csViewOutStuff);
		...
		case 28:	// Screen::putString
		{
			...
			csViewOutStuff.Format( "         nLength = ScreenUTS.putString( cString, nRow, nColumn )");
			pcsaBody->Add(csViewOutStuff);
		...
		case 29:		// IScreenUPG::putStringEx
		{
			...
			pcsaBody->Add("        try");
			pcsaBody->Add("        {");
			pcsaBody->Add("           nLength = ScreenUTS.putStringEx( cString, nRow, nColumn, nEndRow, nEndColumn, 0, nWrap, nMode, bFill )");
			pcsaBody->Add("        }");
		...
		case 30:		// IScreenUPG.sendkeys
		{
			...
			csViewOutStuff.Format( "        ScreenUTS.sendkeys( cText, nType )" );
			pcsaBody->Add(csViewOutStuff);
		}
		break;
		...
		case 31:	// IScreenUPG::setCursor
		{
			...
			pcsaBody->Add("        try" );
			pcsaBody->Add("        {" );
			pcsaBody->Add("           ScreenUTS.setCursor( nRow, nColumn )");
			pcsaBody->Add("        }" );
		...
		case 32:	// IScreenUPG::setCursorColumn
		{
			...
			pcsaBody->Add("        try" );
			pcsaBody->Add("        {" );
			pcsaBody->Add("           bRet = ScreenUTS.setCursorColumn( nColumn )");
			pcsaBody->Add("        }" );
			pcsaBody->Add("        catch (e)" );
			pcsaBody->Add("        {" );
			pcsaBody->Add("           MsgBox e.getMessage()");
		...
		case 33:	// IScreenUPG::setCursorRow
		{
			...
			pcsaBody->Add("        try" );
			pcsaBody->Add("        {" );
			pcsaBody->Add("           bRet = ScreenUTS.setCursorRow( nRow )");
			pcsaBody->Add("        }" );
			pcsaBody->Add("        catch (e)" );
			pcsaBody->Add("        {" );
			pcsaBody->Add("           MsgBox e.getMessage() )");
		...
		case 34:	// IScreenUPG::setDataCapture
		{
			...
			pcsaBody->Add("        ScreenUTS.setDataCapture( bCaptureData )" );
			pcsaBody->Add("");
		...
		case 35:		// IScreenUPG::setgetText_Display
		{
			...
			pcsaBody->Add("        ScreenUTS.setgetText_Display( bTextDisplay )" );
			pcsaBody->Add("");
		...
		case 36:		// IScreenUPG::setMaxDataCaptureLength
		{
			...
			pcsaBody->Add("        ScreenUTS.setMaxDataCaptureLength( nBufferSize )" );
			pcsaBody->Add("");
		...
		{
			pcsaBody->Add("        ScreenUTS.showColorDialog()" );
			pcsaBody->Add("");
		}
		break;
		...
		{
			pcsaBody->Add("        ScreenUTS.showTerminal()" );
			pcsaBody->Add("");
		}
		break;
		...
		case 39:		// IScreenUPG::waitForCursor
		{
			...
			pcsaBody->Add("        try" );
			pcsaBody->Add("        {" );
			pcsaBody->Add("           bRet = ScreenUTS.waitForCursor( nRow, nColumn, nTimeout )");
			pcsaBody->Add("        }" );
			pcsaBody->Add("        catch (e)" );
			pcsaBody->Add("        {" );
			pcsaBody->Add("           MsgBox e.getMessage()");
		...
		case 40:		// IScreenUPG::waitForString
		{
			...
			pcsaBody->Add("        try" );
			pcsaBody->Add("        {" );
			pcsaBody->Add("           bRet = ScreenUTS.waitForString( cText, nRow, nColumn, nWait )");
			pcsaBody->Add("        }" );
			pcsaBody->Add("        catch (e)" );
			pcsaBody->Add("        {" );
			pcsaBody->Add("           MsgBox e.getMessage()");
		...
		case 41:		// IScreenUPG::waitHostQuiet
		{
			...
			pcsaBody->Add("        try" );
			pcsaBody->Add("        {" );
			pcsaBody->Add("           bRet = ScreenUTS.waitHostQuiet( nWait, nSettleTime )");
			pcsaBody->Add("        }" );
			pcsaBody->Add("        catch (e)" );
			pcsaBody->Add("        {" );
			pcsaBody->Add("           MsgBox e.getMessage()");
...
void CAG_AIR_VM_UTS_VBS::finalize(CStringArray* pcsaBody)
{
...
/****************************************************************************DC
	Function	:	CAG_AIR_VM_UTS_VBS::declareVariable()

	Purpose		:	Append variable declarations to the end of the header as 
					needed.

*****************************************************************************/
void CAG_AIR_VM_UTS_VBS::declareVariable(AG_VARIABLE agVariable,
									   CStringArray* pcsaHeader,
									   bool* bValue)
{
	CString csLocal;
...
void CAG_AIR_VM_UTS_VBS::checkDebug(bool b_Debug,
								  CString csFunctionName,
								  CStringArray* pcsaBody)
{
	CString csLocal;
...
void CAG_AIR_VM_UTS_VBS::resetVariableFlags()
{
