...
void CAG_MSI_APP_WS::generateHeader(CStringArray* pcsaHeader,
...
void CAG_MSI_APP_WS::appendBody(int nFunctionNum,
	...
	else
	{
		...
		switch ( nFunctionNum )
		{
			case 0:     //MsiAdvertiseProduct
			{
				...
				csViewOutStuff.Format( "nStatus = installer.MsiAdvertiseProduct( csPackagePath, csScriptPath, csTransforms, %s );", csArg4 );
				pcsaBody->Add(csViewOutStuff);
			...
			case 1:     //MsiAdvertiseProductEx
			{
				...
				csViewOutStuff.Format( "nStatus = installer.MsiAdvertiseProduct( \"%s\", \"%s\", \"%s\", %s, %s, %s );", 
			...
			case 17:     //MsiEnumFeatures
			{
				...
				csViewOutStuff.Format( "bRet = installer.MsiEnumFeatures( csProductName, nFeatureIndex, XXX, YYY );"); 
				pcsaBody->Add(csViewOutStuff);
			...
			case 33:     //MsiGetProductInfo
			{
				...
				if ( !b_csProperty )
				{
					declareVariable( AG_PRODUCT_PROPERTY, pcsaHeader, &b_csProperty );
				}

				if ( !b_csRetString )
				{
					declareVariable( AG_RETURN_STRING, pcsaHeader, &b_csRetString );
				}

				if ( !b_nStringLength )
				{
					declareVariable( AG_STRING_LENGTH, pcsaHeader, &b_nStringLength );
				}
				...
				csViewOutStuff.Format( "csProperty = \"%s\"", pcsaArguments->GetAt(2) );
				pcsaBody->Add(csViewOutStuff);
				
				csViewOutStuff.Format( "csRetString = \"%s\"", pcsaArguments->GetAt(3) );
				pcsaBody->Add(csViewOutStuff);
				
				csViewOutStuff.Format( "nStringLength = %s", pcsaArguments->GetAt(4) );
				pcsaBody->Add(csViewOutStuff);
				
				csViewOutStuff.Format( "uFeatureState = installer.MsiGetProductInfo( csProductName, csProperty, csRetString, nStringLength )"); 
				pcsaBody->Add(csViewOutStuff);
			...
			case 53:     //MsiQueryFeatureState
			{
				...
				csViewOutStuff.Format( "uFeatureState = installer.MsiQueryFeatureState( csProductName, csFeatureName );"); 
				pcsaBody->Add(csViewOutStuff);
			...
			case 73:     //MsiUseFeature
			{
				...
				csViewOutStuff.Format( "uInstallState = installer.MsiUseFeature( csProductName, csFeatureName );"); 
				pcsaBody->Add(csViewOutStuff);
			...
			case 8:     //MsiConfigureFeature
			{
				...
				csViewOutStuff.Format( "uRet = installer.MsiConfigureFeature( csProductName, csFeatureName, uInstallState );"); 
				pcsaBody->Add(csViewOutStuff);
...
void CAG_MSI_APP_WS::finalize(CStringArray* pcsaBody)
{
	if ( b_csTaskID )
	{
	...
	pcsaBody->Add( "" );
	pcsaBody->Add( "Wscript.Quit 0" );
	pcsaBody->Add( "" );

}

void CAG_MSI_APP_WS::declareVariable(AG_VARIABLE agVariable,
										  CStringArray* pcsaHeader,
										  bool* bValue)
{
	CString csLocal;
	*bValue = TRUE;
	...
	switch ( agVariable )
	{
		...
		case AG_XML_DATA:
		{
			csLocal.Format( "Dim XMLData As String");
		}
		break;
		...
		case AG_TRANSFORMS:
		{
			csLocal.Format( "Dim csTransforms As String");
		}
		break;
		...
		case AG_PRODUCT_PROPERTY:
		{
			csLocal.Format( "Dim csProperty As String");
		}
		break;

		case AG_RETURN_STRING:
		{
			csLocal.Format( "Dim csRetString As String");
		}
		break;

		case AG_STRING_LENGTH:
		{
			csLocal.Format( "Dim nStringLength As Ingeger");
		}
		break;
		...
		case AG_UINT_RETURN:
		{
			csLocal.Format( "Dim uRet As Integer");
		}
		break;
...
void CAG_MSI_APP_WS::checkDebug( bool b_Debug, CString csFunctionName, CStringArray* pcsaBody )
{
	CString csLocal;
	
	if ( b_Debug )
	{
		csLocal.Format( "    Wscript.Echo \"%s\"", csFunctionName );
		pcsaBody->Add(csLocal);
	}
}


void CAG_MSI_APP_WS::resetVariableFlags()
{
	b_csTaskID=FALSE;
	b_csBuffer=FALSE;
	b_nBufferSize=FALSE;
...
void CAG_MSI_APP_WS::loadLanguageList(CListBox* listTranslateLanguage)
{
	int nPos = 0;
...
void CAG_MSI_APP_WS::loadTerminalTypeList(CListBox* listTerminalTypes)
{
	int nPos = 0;
...
void CAG_MSI_APP_WS::loadFunctionList(CListBox* listTranslateFunction)
{
