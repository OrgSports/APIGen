...
CAG_MCS_J_3270_VBS_HDR::CAG_MCS_J_3270_VBS_HDR()
{
	resetVariableFlags();	
}
...
void CAG_MCS_J_3270_VBS_HDR::generateHeader(CStringArray* pcsaHeader,
										  CStringArray* pcsaBody)
{
	...
	csViewOutStuff.Format( "'          Attachmate APIGen VBScript MCS Footer");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "'");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "'  This is a VBScript MCS footer file that was generated to automate");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "'  an Attachmate MCS Java viewer.  It is dependent on an Initialize()");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "'  subroutine that you must invoke via a matching header file's");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "'   onLoad() method within the header's <BODY> tag.");
	pcsaHeader->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "'   <BODY BGCOLOR=\"#C0C0C0\" Language=VBScript onLoad=\"Initialize\">");
	pcsaHeader->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "'  If you have questions about how to prepare MCS header and footer");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "'  files, please consult the Attachmate JavaScript Guide for MCS Java Viewers.");
	pcsaHeader->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "'   Notes       :   This function is to be called via the onLoad specifier in ");
	pcsaBody->Add(csViewOutStuff);

	csViewOutStuff.Format( "'                   an MCS header file.");
	pcsaBody->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "   display = terminal.getDisplay()");
	pcsaBody->Add(csViewOutStuff);
...
void CAG_MCS_J_3270_VBS_HDR::appendBody(int nFunctionNum,
...
void CAG_MCS_J_3270_VBS_HDR::finalize(CStringArray* pcsaBody)
{
	CString csViewOutStuff;
...
/****************************************************************************DC
	Function	:	CAG_MCS_J_3270_VBS_HDR::declareVariable()

	Purpose		:	Append variable declarations to the end of the header as 
					needed.

*****************************************************************************/
void CAG_MCS_J_3270_VBS_HDR::declareVariable(AG_VARIABLE agVariable,
										   CStringArray* pcsaHeader,
										   bool* bValue)
{
	CString csLocal;
...
void CAG_MCS_J_3270_VBS_HDR::checkDebug(bool b_Debug,
...
void CAG_MCS_J_3270_VBS_HDR::resetVariableFlags()
{
   b_pOIAObject = FALSE;
   b_nStatus = FALSE;
   b_nType = FALSE;
