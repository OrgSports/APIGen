...
CAG_EOLE_3270_NODE_JS::CAG_EOLE_3270_NODE_JS()
{
	resetVariableFlags();
}
...
void CAG_EOLE_3270_NODE_JS::generateHeader(CStringArray* pcsaHeader,
											CStringArray* pcsaBody)
{
	...
	csViewOutStuff.Format( "%s", "//                         APIGen JavaScript");
	pcsaHeader->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "%s", "//   This HTML file consists of JavaScript code that loads the Extra! X-treme");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "%s", "//   ActiveX control.");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "//" );
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "%s", "//   To use this code: ");
	pcsaHeader->Add(csViewOutStuff);
	...
	pcsaHeader->Add("<html>");
	pcsaHeader->Add("<head>");
	pcsaHeader->Add("<title>Extra! X-treme Launcher -- APIGen JavaScript</title>");
	pcsaHeader->Add("");
	pcsaHeader->Add("<script language=\"JavaScript\">");
	pcsaHeader->Add("");
	pcsaHeader->Add("function startextra()");
	pcsaHeader->Add("{");
	pcsaHeader->Add("  var sessionname");
	pcsaHeader->Add("");
	pcsaHeader->Add("  sessionname = inputbox(\"EXTRA! session to open:\", \"EXTRA!\", \"IMS.edp\")");
	pcsaHeader->Add("  if ( sessionname <> \"\" )");
	pcsaHeader->Add("  {");
	...
	pcsaHeader->Add("");
	pcsaHeader->Add("     if ( extra.launch() != null )");
	pcsaHeader->Add("     {");
	...
	pcsaHeader->Add("");
	pcsaHeader->Add("");
	pcsaHeader->Add("function APIGenerated()");
	pcsaHeader->Add("{");
	pcsaBody->Add("");
...
void CAG_EOLE_3270_NODE_JS::appendBody(int nFunctionNum,
										...
										CStringArray* pcsaBody)
{
	...
	case 5:		// Screen::Col
		...
		case 19:  //WaitForStream
		{
			...
			pcsaBody->Add("   bRet = extra.session.screen.WaitForStream(cString, nIdle, nTimeout, nOptions)" );

		}
		break;	


		case 20:	// WaitForString	donc101905 if endcol/endrow are blank don't put them in the command
		{
			if ( !b_nColumn )
			{
				declareVariable( AG_COLUMN, pcsaHeader, &b_nColumn );
			...
			pcsaBody->Add(csViewOutStuff);
			csViewOutStuff.Format("   nType = %s", pcsaArguments->GetAt(7));
			pcsaBody->Add(csViewOutStuff);
			...
			if ( 0 != pcsaArguments->GetAt(8).GetLength() || 0 != pcsaArguments->GetAt(9).GetLength() )
			{
				...
				pcsaBody->Add("   bRet = extra.session.screen.WaitForString(cString, nRow, nColumn, nPage, nIdle, nTimeout, nOptions, nType, nEndRow, nEndColumn)" );
			}
			else
			{
				pcsaBody->Add("   bRet = extra.session.screen.WaitForString(cString, nRow, nColumn, nPage, nIdle, nTimeout, nOptions, nType )" );
			}
		...
		case 39:	// Session::KeyMap
		{
			...
			if ( -1 != pcsaArguments->GetAt(1).Find( "TRUE" ) )
			{
				...
				pcsaBody->Add(csViewOutStuff);
				pcsaBody->Add("   extra.session.KeyMap=cString");

			}
			else
			{
				
				pcsaBody->Add("   cString = extra.session.KeyMap");
			}
		...
		{
			csViewOutStuff.Format( "   extra.session.ReduceFont" );
			pcsaBody->Add(csViewOutStuff);
		}
		break;
		...
		{
			csViewOutStuff.Format( "   extra.session.Reset" );
			pcsaBody->Add(csViewOutStuff);
		}
		break;
		...
		{
			csViewOutStuff.Format( "   extra.session.Save" );
			pcsaBody->Add(csViewOutStuff);
		}
		break;
		...
		case 51:	// Session::Saved
		{
			...
			csViewOutStuff.Format( "   bSessionSaved = extra.session.Saved" );
			pcsaBody->Add(csViewOutStuff);
		}
		break;
		...
		{
			csViewOutStuff.Format( "   extra.session.TransferFile" );
			pcsaBody->Add(csViewOutStuff);
		}
		break;
...
void CAG_EOLE_3270_NODE_JS::finalize(CStringArray* pcsaBody)
{
	pcsaBody->Add("");
	pcsaBody->Add("}");
	pcsaBody->Add("");
...
void CAG_EOLE_3270_NODE_JS::declareVariable(AG_VARIABLE agVariable,
											 CStringArray* pcsaHeader,
											 bool* bValue)
{
	CString csLocal;
	*bValue = TRUE;
	...
	switch ( agVariable )
	{
		...
		case AG_FT_OBJ:
		{
			csLocal.Format( "   var ftObj");
		}
		break;

		case AG_HOSTNAME:
		{
			csLocal.Format( "   var hostname");
		}
		break;
		...
		case AG_SCREEN_HELD:
		{
			csLocal.Format( "   var bScreenHeld");
		}
		break;
		...
		{
			csLocal.Format( "   var settingsPage");
		}
		break;

		case AG_SETTINGS_TAB:
		{
			csLocal.Format( "   var settingsTab");
		}
		break;
		...
		case AG_IDLE_TIME:
		{
			csLocal.Format( "   var nIdle");
		}
		break;

		case AG_OPTIONS:
		{
			csLocal.Format( "   var nOptions");
		}
		break;

		case AG_PAGE:
		{
			csLocal.Format( "   var nPage");
		}
		break;
		...
		case AG_PROCESS:
		{
			csLocal.Format( "   var bProcess");
		}
		break;

		case AG_PROCESS_KEYSTROKES:
		{
			csLocal.Format( "   var sKeys");
		}
		break;
		...
		case AG_FONTNAME:
		{
			csLocal.Format( "   var font" );
		}
		break;

		case AG_FT_SCHEME:
		{
			csLocal.Format( "   var FTScheme" );
		}
		break;
...
void CAG_EOLE_3270_NODE_JS::checkDebug(bool b_Debug,
...
void CAG_EOLE_3270_NODE_JS::resetVariableFlags()
{
	b_cString=FALSE;
	b_Area=FALSE;
	b_bRet=FALSE;
