...
CAG_MSI_DBASE_VBS::CAG_MSI_DBASE_VBS()
{
	APIFlavor=MSI_DB_API;
	resetVariableFlags();	
}
...
void CAG_MSI_DBASE_VBS::generateHeader(CStringArray* pcsaHeader,
										 CStringArray* pcsaBody)
{
	...
	pcsaHeader->Add("'               MSI Installer Application");
	pcsaHeader->Add("' ");
...
void CAG_MSI_DBASE_VBS::appendBody(int nFunctionNum,
...
void CAG_MSI_DBASE_VBS::finalize(CStringArray* pcsaBody)
{
	if ( b_csTaskID )
	{
...
void CAG_MSI_DBASE_VBS::declareVariable(AG_VARIABLE agVariable,
										  CStringArray* pcsaHeader,
										  bool* bValue)
{
	CString csLocal;
...
void CAG_MSI_DBASE_VBS::checkDebug( bool b_Debug, CString csFunctionName, CStringArray* pcsaBody )
{
	CString csLocal;
...
void CAG_MSI_DBASE_VBS::resetVariableFlags()
{
	b_csTaskID=FALSE;
	b_csBuffer=FALSE;
	b_nBufferSize=FALSE;
...
void CAG_MSI_DBASE_VBS::loadLanguageList(CListBox* listTranslateLanguage)
{
	int nPos = 0;
...
void CAG_MSI_DBASE_VBS::loadTerminalTypeList(CListBox* listTerminalTypes)
{
	int nPos = 0;
...
void CAG_MSI_DBASE_VBS::loadFunctionList(CListBox* listTranslateFunction)
{
	...
	csaFunctions.Add("MsiCreateTransformSummaryInfo");
	csaFunctions.Add("MsiDatabaseApplyTransform");
	csaFunctions.Add("MsiDatabaseCommit");
	csaFunctions.Add("MsiDatabaseExport");
	csaFunctions.Add("MsiDatabaseGenerateTransform");
	csaFunctions.Add("MsiDatabaseGetPrimaryKeys");
	csaFunctions.Add("MsiDatabaseImport");
	csaFunctions.Add("MsiDatabaseIsTablePersistent");
	csaFunctions.Add("MsiDatabaseMerge");
	csaFunctions.Add("MsiDatabaseOpenView");
	csaFunctions.Add("MsiDoAction");
	csaFunctions.Add("MsiEnableUIPreview");
