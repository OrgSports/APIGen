...
void CAG_IHL_VBN::generateHeader(CStringArray* pcsaHeader,
								 CStringArray* pcsaBody)
{
	...
	csViewOutStuff.Format( "// that is ready to build and run.  To do so, close any open Visual C++ project,");
	pcsaHeader->Add(csViewOutStuff);
	
	csViewOutStuff.Format( "// open this file, then select the Build\\Compile menu item.");
	pcsaHeader->Add(csViewOutStuff);
	
	csViewOutStuff.Format( "// Accept the prompt to start a new project and compilation will proceed.");
	pcsaHeader->Add(csViewOutStuff);
...
void CAG_IHL_VBN::appendBody(int nFunctionNum,
...
void CAG_IHL_VBN::finalize(CStringArray* pcsaBody)
{
		CString csLocal;
...
void CAG_IHL_VBN::declareVariable(AG_VARIABLE agVariable,
								  CStringArray* pcsaHeader,
								  bool* bValue)
{
	CString csLocal;
...
void CAG_IHL_VBN::checkDebug(bool b_Debug,
...
void CAG_IHL_VBN::resetVariableFlags()
{
