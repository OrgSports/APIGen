...
#include "CAG_AIR_5250_J.h"

void CAG_AIR_5250_J::checkDebug()
{
	
}

CAG_AIR_5250_J::CAG_AIR_5250_J()
{
	APIFlavor = AIRJ_API;	
}
...
void CAG_AIR_5250_J::generateHeader(CStringArray* pcsaHeader,
									CStringArray* pcsaBody)
{
	...
	csViewOutStuff.Format( "//             Attachmate APIGen Airlines SDK 5250 Java Application");
	pcsaHeader->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "//    of Airlines SDK 5250 Applet Java method calls specified to the APIGen code");
	pcsaHeader->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "//       1. Either the class name (Air5250) or the file name must be changed as");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "//          they must be the same.");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "//       2. The Attachmate Airlines SDK must be installed using the keycode for 5250. Its");
	pcsaHeader->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "//       4. You must prepare a 5250 session XML configuration file that the");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "//           generated program can use to configure its connection.");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "//           One example of such a configuration file is 5250.xml which");
	pcsaHeader->Add(csViewOutStuff);
	...
	pcsaHeader->Add(csViewOutStuff);
 	...
 	csViewOutStuff.Format( "public class Air5250 implements IScreenListener");
	pcsaHeader->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "      Air5250 a5250 = new Air5250();" );
	pcsaBody->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "           a5250.processHost();" );
	pcsaBody->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "             fis = new FileInputStream(\"C:\\temp\\5250.xml\");" );
	pcsaBody->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "        ScreenTn5250 so;" );
	pcsaBody->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "             so = (ScreenTn5250) sl.requestScreenEx(\"\",\"\",strConfig,0);" );
	pcsaBody->Add(csViewOutStuff);
...
void CAG_AIR_5250_J::appendBody(int nFunctionNum,
...
void CAG_AIR_5250_J::finalize(CStringArray* pcsaBody)
{}

void CAG_AIR_5250_J::declareVariable(AG_VARIABLE agVariable,
									 CStringArray* pcsaHeader,
									 bool* bValue)
{
