...
CAG_EOLE_3270_XLS::CAG_EOLE_3270_XLS()
{
	resetVariableFlags();	
}
...
void CAG_EOLE_3270_XLS::generateHeader(CStringArray* pcsaHeader,
									   CStringArray* pcsaBody)
{
	...
	csViewOutStuff.Format( "%s", "'             Attachmate APIGen");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "%s", "'  Windows VBA Scripting 3270 Host OLE Automation");
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "%s", "'" );
	pcsaHeader->Add(csViewOutStuff);

	csViewOutStuff.Format( "%s", "'   This file consists of a \"Main\" subroutine prepared for Windows Scripting");
	pcsaHeader->Add(csViewOutStuff);
...
/****************************************************************************DC
	Function	:	CAG_EOLE_3270_XLS::appendBody()
...
*****************************************************************************/
void CAG_EOLE_3270_XLS::appendBody(int nFunctionNum,
...
void CAG_EOLE_3270_XLS::finalize(CStringArray* pcsaBody)
{
	pcsaBody->Add("");	
...
void CAG_EOLE_3270_XLS::declareVariable(AG_VARIABLE agVariable,
										CStringArray* pcsaHeader,
										bool* bValue)
{
	CString csLocal;
...
void CAG_EOLE_3270_XLS::checkDebug(bool b_Debug,
...
void CAG_EOLE_3270_XLS::resetVariableFlags()
{
   b_SessionOpen = FALSE;
   b_nAttribute = FALSE;
   b_nStatus = FALSE;
