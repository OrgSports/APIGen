...
CAG_EOLE_3270_WS::CAG_EOLE_3270_WS()
{
	resetVariableFlags();
}
...
void CAG_EOLE_3270_WS::generateHeader(CStringArray* pcsaHeader,
									   CStringArray* pcsaBody)
{
	...
	pcsaHeader->Add("' ****************************************************************************");
	pcsaHeader->Add("'             Attachmate Extra! 3270 OLE Automation");
	pcsaHeader->Add("'              APIGen Windows Scripting with VBScript");
	pcsaHeader->Add("'" );
	pcsaHeader->Add("'   This file consists of Windows VBScript commands that drive the Extra! X-treme");
	pcsaHeader->Add("'   OLE Automation interface.");
	pcsaHeader->Add("'" );
	...
	pcsaBody->Add("bRet = Sess0.screen.WaitForString(cString, nRow, nColumn, nPage, nIdle, nTimeout, nOptions, nType )" );
	pcsaBody->Add("" );
...
/****************************************************************************DC
	Function	:	CAG_EOLE_3270_WS::appendBody()
...
*****************************************************************************/
void CAG_EOLE_3270_WS::appendBody(int nFunctionNum,
								   ...
								   CStringArray* pcsaBody)
{
	...
	switch ( nFunctionNum )
	{
		...
		{		
			...
			{
				pcsaBody->Add("Sess0.screen.OIA.Value = cString;");
			}
			else
			{
				pcsaBody->Add("cString = Sess0.screen.OIA.Value");
			}
		}
		break;
	...
	case 5:		// Screen::Col
		...
		{			
			...
			pcsaBody->Add("cString = Sess0.screen.GetString(nRow, nColumn, nLength)");			
		}
		break;
		...
		{			
			...
			pcsaBody->Add("Sess0.screen.PutString cString, nRow, nColumn" );
		...
		case 12:	// Screen.Search  
		{
			...
			pcsaBody->Add("set Area = Sess0.screen.Search(cString, nRow, nColumn) " );
		...
		case 14:	// Screen.SendKeys
		{
			...
			csViewOutStuff.Format( "cString = \"%s\"", pcsaArguments->GetAt(1) );
			pcsaBody->Add(csViewOutStuff);
			csViewOutStuff.Format( "Sess0.screen.Sendkeys( cString )" );
			pcsaBody->Add(csViewOutStuff);
		...
		case 19:  //WaitForStream
		{
			...
			pcsaBody->Add("bRet = Sess0.screen.WaitForStream(cString, nIdle, nTimeout, nOptions)" );
		...
		case 20:	// WaitForString
		{
			...
			if ( 0 != pcsaArguments->GetAt(8).GetLength() || 0 != pcsaArguments->GetAt(9).GetLength() )
			{
				...
				pcsaBody->Add("bRet = Sess0.screen.WaitForString(cString, nRow, nColumn, nPage, nIdle, nTimeout, nOptions, nType, nEndRow, nEndColumn)" );
			}
			else
			{
				pcsaBody->Add("bRet = Sess0.screen.WaitForString(cString, nRow, nColumn, nPage, nIdle, nTimeout, nOptions, nType )" );
		...
		case 40:	// Session::Language
		{
			...
			if ( -1 != pcsaArguments->GetAt(1).Find( "TRUE" ) )
			{
				csViewOutStuff.Format( "cString = \"%s\"", pcsaArguments->GetAt(2) );
				pcsaBody->Add(csViewOutStuff);
				pcsaBody->Add("Sess0.Language = cString" );
			}
			else
			{
				pcsaBody->Add( "   cString = Sess0.Language");
			}
		...
		case 43:	// Session::Name
		{
			...
			pcsaBody->Add( "   cString = Sess0.Name");
		}
		break;
		...
		case 53:	// Session::Title
		{
			...
			csViewOutStuff.Format( "Sess0.Title( cString )");
			pcsaBody->Add(csViewOutStuff);
		}
		break;
		...
		case 56:	// Session::UpdateStatusBar
		{
			...
			csViewOutStuff.Format( "Sess0.UpdateStatusBar( cString )");
			pcsaBody->Add(csViewOutStuff);
		}
		break;
		...
		{			
			...
			pcsaBody->Add( "   cString = sys.Version");
		}
		break;
...
void CAG_EOLE_3270_WS::finalize(CStringArray* pcsaBody)
{
	pcsaBody->Add("");	
...
void CAG_EOLE_3270_WS::declareVariable(AG_VARIABLE agVariable,
										CStringArray* pcsaHeader,
										bool* bValue)
{
	CString csLocal;
...
void CAG_EOLE_3270_WS::checkDebug(bool b_Debug,
...
void CAG_EOLE_3270_WS::resetVariableFlags()
{
	b_cString=TRUE;
	b_Area=FALSE;
	b_bRet=TRUE;
