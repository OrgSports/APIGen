...
CAG_EAOAx_VT_HTML_VBS::CAG_EAOAx_VT_HTML_VBS()
{
	resetVariableFlags();	
}
...
void CAG_EAOAx_VT_HTML_VBS::generateHeader(CStringArray* pcsaHeader,
											 CStringArray* pcsaBody)
{
		...
		csViewOutStuff.Format( "'       Attachmate APIGen Enterprise Access Objects ");
		pcsaHeader->Add(csViewOutStuff);
	
		csViewOutStuff.Format( "'      VT Terminal ActiveX VBScript HTML Application");
		pcsaHeader->Add(csViewOutStuff);

		csViewOutStuff.Format( " " );
		pcsaHeader->Add(csViewOutStuff);

		csViewOutStuff.Format( "'  NOTE THIS: THIS CODE IS NOT READY." );
		pcsaHeader->Add(csViewOutStuff);
		...
		csViewOutStuff.Format( "<title>Attachmate EAO ActiveX Object Using VBScript</title>" );
		pcsaHeader->Add(csViewOutStuff);
		...
		csViewOutStuff.Format( "Sub onConnect()");
		pcsaHeader->Add(csViewOutStuff);
		...
		csViewOutStuff.Format( "Sub onDisconnect()" );
		pcsaHeader->Add(csViewOutStuff);
		...
		csViewOutStuff.Format( "Sub RunAPIGen()" );
		pcsaHeader->Add(csViewOutStuff);
...
void CAG_EAOAx_VT_HTML_VBS::appendBody(int nFunctionNum,
...
void CAG_EAOAx_VT_HTML_VBS::finalize(CStringArray* pcsaBody)
{
		CString csLocal;
...
void CAG_EAOAx_VT_HTML_VBS::declareVariable(AG_VARIABLE agVariable,
											  CStringArray* pcsaHeader,
											  bool* bValue)
{
	CString csLocal;
	*bValue = TRUE;
	...
	switch ( agVariable )
	{
		case AG_ATMFT:
		{
			csLocal.Format( "   Dim pIAtmFileTransfer");
		}
		break;

		case AG_HRESULT:
		{
			csLocal.Format( "   Dim hr" );
		}
		break;
...
void CAG_EAOAx_VT_HTML_VBS::checkDebug(bool b_Debug,
...
void CAG_EAOAx_VT_HTML_VBS::resetVariableFlags()
{
   b_nHRESULT = FALSE;
   b_nFLAG = FALSE;
   b_nATMFT = FALSE;
