...
CAG_AIR_3270_J_J::CAG_AIR_3270_J_J()
{
	resetVariableFlags();
}
...
void CAG_AIR_3270_J_J::generateHeader(CStringArray* pcsaHeader,
									  CStringArray* pcsaBody)
{
	...
	if (IDOK == XML3270Dlg.DoModal())
	{
		CString csLocalString;
		pcsaHeader->Add("/* 3270.XML Airlines SDK Configuration File");

		CString csCharCase, csBackSpace, csLightPen;

		// if the character case option is specified as something other than mixed
		if ( -1 == XML3270Dlg.csList3270CharCase.Find( "Mixed" ) )
		{
			if ( -1 != XML3270Dlg.csList3270CharCase.Find( "Lower" ) )
			{
				csCharCase="Lower";
			}
			else
			...
			if ( -1 != XML3270Dlg.csList3270BackSpace.Find( "delete" ) )
			{
				csBackSpace="Delete";

				// if the lightpen is to be enabled
				if ( XML3270Dlg.bCheck3270LightPen )
				{
					csLocalString.Format( "<ScreenTn3270 TextCase=\"%s\" LPenEnabled=\"True\" BackSpaceKey=\"%s\">", csCharCase, csBackSpace );
				}
				else
				{
					csLocalString.Format( "<ScreenTn3270 TextCase=\"%s\" BackSpaceKey=\"%s\">", csCharCase, csBackSpace );
				}
			}
			else
			{
				...
				if ( XML3270Dlg.bCheck3270LightPen )
				{
					csLocalString.Format( "<ScreenTn3270 TextCase=\"%s\" LPenEnabled=\"True\">", csCharCase );
				}
				else
				{
					csLocalString.Format( "<ScreenTn3270 TextCase=\"%s\">", csCharCase );
				}
		...
		else
		{
			...
			if ( -1 != XML3270Dlg.csList3270BackSpace.Find( "delete" ) )
			{
				...
				if ( XML3270Dlg.bCheck3270LightPen )
				{
					csLocalString.Format( "<ScreenTn3270 LPenEnabled=\"True\" BackSpaceKey=\"%s\">", csBackSpace );
				}
				else
				{
					csLocalString.Format( "<ScreenTn3270 BackSpaceKey=\"%s\">", csBackSpace );
				}
			}
			else
			{
				...
				if ( XML3270Dlg.bCheck3270LightPen )
				{
					csLocalString.Format( "<ScreenTn3270 LPenEnabled=\"True\">");
				}
				else
				{
					csLocalString.Format( "<ScreenTn3270>" );
				}
			}
		...
		pcsaHeader->Add("        <Tn3270con>");
		csLocalString.Format( "               <Tcp3270con HostName=\"%s\" PortNumber=\"%s\"\\>", XML3270Dlg.csEdit3270HostName, XML3270Dlg.csEdit3270PortNumber );
		pcsaHeader->Add(csLocalString);
		pcsaHeader->Add("        <\\Tn3270con>");
		csLocalString.Format( "        <ReadXlat XlateFileName=\"%s\"\\>", XML3270Dlg.csEdit3270XLateFileName );
		pcsaHeader->Add(csLocalString);
		pcsaHeader->Add("<\\ScreenTn3270>");
		pcsaHeader->Add(" *** END 3270.XML Airlines SDK Configuration File ***/ ");
		pcsaHeader->Add("");
		pcsaHeader->Add("");
	}   
	...
	pcsaHeader->Add("// ****************************************************************************");
	pcsaHeader->Add("//                Attachmate Airlines 3270 SDK");
	...
	pcsaHeader->Add("//    of Airlines SDK 3270 Applet Java method calls specified to the APIGen code");
	pcsaHeader->Add("//    generator.");
	pcsaHeader->Add("//");
	pcsaHeader->Add("//    These conditions must be met before this source file can be used.");
	pcsaHeader->Add("//       1. Either the class name (Air3270) or the file name must be changed as");
	pcsaHeader->Add("//          they must be the same.");
	pcsaHeader->Add("//       2. The Attachmate Airlines SDK must be installed using the keycode for 3270. Its");
	pcsaHeader->Add("//           class or jar files must be made accessible via the class path.");
	pcsaHeader->Add("//       3. If you specified the parameters for a 3270 session XML configuration file");
	...
	pcsaHeader->Add("//          to its own file and either place the new file named, \"3270.xml\" to the c:\\temp");
	...
	pcsaHeader->Add("         fis = new FileInputStream(\"C:\\temp\\3270.xml\");" );
	...
	pcsaHeader->Add("      ScreenTn3270 screen;" );
	pcsaHeader->Add("      String strConfig = getXmlConfig();" );
	pcsaHeader->Add("      System.out.println(\"DEBUG:    strConfig = \" + strConfig);" );
	pcsaHeader->Add("" );
	pcsaHeader->Add("      try" );
	pcsaHeader->Add("      {" );
	pcsaHeader->Add("         screen = (ScreenTn3270) sessLoader.requestScreenEx(\"\",\"\",strConfig,0);" );
...
void CAG_AIR_3270_J_J::appendBody(int nFunctionNum,
								  ...
								  CStringArray* pcsaBody)
{
	...
	switch ( nFunctionNum )
	{
		...
		case 5:		// IScreenUPG::addIScreenListener
		{
			...
			pcsaBody->Add("              ScreenTn3270 = (ScreenType) sessLoader.requestScreen(User,Password,WebView);");
			pcsaBody->Add("              if (ScreenTn3270 != null)");
			pcsaBody->Add("              {");
			pcsaBody->Add("                   ScreenTn3270.addIScreenListener(this);");
			pcsaBody->Add("                   if (ScreenTn3270.Open())");
			pcsaBody->Add("                   {");
			pcsaBody->Add("                   }");
			pcsaBody->Add("              }");
			pcsaBody->Add(" " );
		...
		{
			pcsaBody->Add("            ScreenTn3270 = (ScreenType) sessLoader.requestScreen(User,Password,WebView); //XXX declare vars");
			pcsaBody->Add("            if (ScreenTn3270 != null)");
			pcsaBody->Add("            {");
			pcsaBody->Add("                 ScreenTn3270.clearCapturedData();");
			pcsaBody->Add("            }");
			pcsaBody->Add(" " );
		}
		break;
		...
		case 8:		// IScreenUPG::fieldCount
		{
			...
			{
				nLocalField=3;
				csViewOutStuff.Format( "            nCount = screen.fieldCount( %i );  //FIELDTYPE_ANY", nLocalField );
			}
			pcsaBody->Add(csViewOutStuff);
			pcsaBody->Add(" " );
		}
		...
		case 9:		// IScreenUPG::findField
		{
			...
			else if ( -1 != pcsaArguments->GetAt(1).Find( "FINDFIELD_NEXT" ) )
			{
				nLocalMode=1;
		...
		case 45:		// IScreenUPG::setgetText_Display
{
		...
		case 58:	// SessionLoader::setTerminalType
		{
			...
			else if ( -1 != pcsaArguments->GetAt(1).Find( "3270" ) )
			{
				csViewOutStuff.Format( "            nType = 110; //3270" );
			}
...
void CAG_AIR_3270_J_J::finalize(CStringArray* pcsaBody)
{
	CString csViewOutStuff;
...
/****************************************************************************DC
	Function	:	CAG_AIR_3270_J_J::declareVariable()

	Purpose		:	Append variable declarations to the end of the header as 
					needed.

*****************************************************************************/
void CAG_AIR_3270_J_J::declareVariable(AG_VARIABLE agVariable,
									   CStringArray* pcsaHeader,
									   bool* bValue)
{
	CString csLocal;
...
void CAG_AIR_3270_J_J::checkDebug(bool b_Debug,
								  CString csFunctionName,
								  CStringArray* pcsaBody)
{
...
void CAG_AIR_3270_J_J::resetVariableFlags()
{
