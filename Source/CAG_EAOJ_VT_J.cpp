...
void CAG_EAOJ_VT_J::generateHeader(CStringArray* pcsaHeader,
									 CStringArray* pcsaBody)
{
	...
	csViewOutStuff.Format( "//              EAO for Java VT Java Application");
	pcsaHeader->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "         ScreenAsync screen=null;" );
	pcsaBody->Add(csViewOutStuff);

	csViewOutStuff.Format( "         ConTNAsync conn=null;" );
	pcsaBody->Add(csViewOutStuff);

	csViewOutStuff.Format( "         DisplayAsyncRenderer display=null;" );
	pcsaBody->Add(csViewOutStuff);

	csViewOutStuff.Format( "         StatusAsyncRenderer status=null;" );
	pcsaBody->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "            conn = new ConTNAsync();" );
	pcsaBody->Add(csViewOutStuff);
	...
	csViewOutStuff.Format( "            screen = new ScreenAsync();;" );
	pcsaBody->Add(csViewOutStuff);

	csViewOutStuff.Format( "            display = new DisplayAsyncRenderer();" );
	pcsaBody->Add(csViewOutStuff);

	csViewOutStuff.Format( "            status = new StatusAsyncRenderer();" );
	pcsaBody->Add(csViewOutStuff);
...
void CAG_EAOJ_VT_J::appendBody(int nFunctionNum,
...
void CAG_EAOJ_VT_J::finalize(CStringArray* pcsaBody)
{
	CString csViewOutStuff;
...
*****************************************************************************/
void CAG_EAOJ_VT_J::declareVariable(AG_VARIABLE agVariable,
									  CStringArray* pcsaHeader,
									  bool* bValue)
{
	CString csLocal;
	*bValue = TRUE;
	...
	switch ( agVariable )
	{
		...
		case AG_OIAOBJ:
		{
			csLocal.Format( "    OiaAsync OIAObj;" );
		}
		break;
		...
		{
			csLocal.Format( "    ConTNAsync conn;" );
		}
		break;

		case AG_SCREEN_OBJ:
		{
			csLocal.Format( "    ScreenAsync screen;" );
		}
		break;
...
void CAG_EAOJ_VT_J::checkDebug( bool b_Debug, CString csFunctionName, CStringArray* pcsaBody )
{
	CString csLocal;
...
void CAG_EAOJ_VT_J::resetVariableFlags()
{
   b_pOIAObject = FALSE;
   b_nStatus = FALSE;
   b_nType = FALSE;
